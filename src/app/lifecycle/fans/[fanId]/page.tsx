...
import { FanMemoryCard } from "@/components/lifecycle/fan-memory-card";
import { NextBestActionCard } from "@/components/lifecycle/next-best-action-card";
...
export default async function FanLifecycleProfile({ params }: { params: Promise<{ fanId: string }> }) {
  ...
  const fan = await prisma.fan.findFirst({
    ...
    include: {
      ...
      memories: { orderBy: [{ confirmedAt: "desc" }, { observedAt: "desc" }], take: 50 },
  ...
  const activeMemories = fan.memories.filter((memory) => memory.status === "ACTIVE");
  const dismissedMemoryKeys = new Set(fan.memories.filter((memory) => memory.status === "DISMISSED").map((memory) => `${memory.category}:${memory.key}`));
  const detectedMemories = activeMemories.length ? [] : deriveFanMemory(inboundMessages.map((message) => ({ id: message.id, text: message.text, sentAt: message.sentAt })))
    .filter((memory) => !dismissedMemoryKeys.has(`${memory.category}:${memory.key}`));
  const effectiveMemories = activeMemories.length ? activeMemories : detectedMemories;
  const nextAction = calculateNextBestAction({
    ...
    lastInboundAt: inboundMessages[0]?.sentAt ?? null,
    lastOutboundAt: outboundMessages[0]?.sentAt ?? null,
    hasActiveWorkflow: Boolean(activeEnrollment),
  ...
  return <div className="flex min-h-screen bg-[#101218] text-zinc-100"><Sidebar/><div className="min-w-0 flex-1"><Topbar/>
    <main id="main-content" className="mx-auto max-w-[1500px] px-4 py-8 md:px-8">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]"><div className="min-w-0 space-y-6">
        ...
        <NextBestActionCard fanId={fan.id} action={nextAction}/>
        <section className="rounded-2xl border border-white/8 bg-white/[.025] p-5"><div className="flex items-end justify-between gap-4"><div><h2 className="font-semibold text-white">Timeline unificado</h2><p className="mt-1 text-xs text-zinc-600">Mensajes, compras, suscripciones, etapas y automatizaciones.</p></div><span className="text-xs text-zinc-600">{timeline.length} eventos</span></div><div className="mt-5 max-h-[680px] space-y-0 overflow-y-auto overscroll-contain pr-2">{timeline.map((item) => <article key={item.id} className="relative grid grid-cols-[18px_1fr] gap-3 pb-5 last:pb-0"><div className="relative flex justify-center"><span className={`relative z-10 mt-1.5 size-2 rounded-full ${item.tone}`}/><span className="absolute bottom-0 top-3 w-px bg-white/8 last:hidden"/></div><div><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-medium text-zinc-300">{item.title}</p><time className="text-[10px] text-zinc-700">{item.at.toLocaleString("es-MX")}</time></div><p className="mt-1 whitespace-pre-wrap break-words text-xs leading-5 text-zinc-600">{item.detail}</p></div></article>)}</div></section>
      </div><div className="space-y-6"><FanMemoryCard fanId={fan.id} memories={effectiveMemories.map((memory) => ({ ...memory, id: "id" in memory ? memory.id : null, observedAt: memory.observedAt.toISOString() }))}/><FanProfileActions fanId={fan.id} stage={fan.lifecycleStage} override={fan.lifecycleOverride} automationPaused={fan.automationPaused} hasActiveWorkflow={Boolean(activeEnrollment)} tags={fan.tags.map(({ tag }) => tag)} notes={fan.notes.map((note) => ({ ...note, createdAt: note.createdAt.toISOString() }))}/></div></div>
    </main>
  </div></div>;
}
