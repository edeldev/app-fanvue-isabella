...
export default async function FanLifecycleProfile({ params }: { params: Promise<{ fanId: string }> }) {
  ...
  return <div className="flex min-h-screen bg-[#101218] text-zinc-100"><Sidebar/><div className="min-w-0 flex-1"><Topbar/>
    <main id="main-content" className="mx-auto max-w-[1500px] px-4 py-8 md:px-8">
      ...
      </div><FanProfileActions fanId={fan.id} stage={fan.lifecycleStage} override={fan.lifecycleOverride} automationPaused={fan.automationPaused} hasActiveWorkflow={Boolean(activeEnrollment)} tags={fan.tags.map(({ tag }) => tag)} notes={fan.notes.map((note) => ({ ...note, createdAt: note.createdAt.toISOString() }))}/></div>
    </main>
  </div></div>;
}
