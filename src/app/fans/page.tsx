...
import { buildFansWhere, fanFilterOptions, parseFanFilter, type FanFilter } from "@/domain/fans/filters";
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm">
            ...
            <tbody>{fans.map((fan) => {
              ...
              return <tr key={fan.id} className="border-b border-white/5 transition hover:bg-white/[.025] last:border-0">
                ...
                <td className="px-5 py-4"><div className="flex flex-wrap gap-2">{fan.isOnline ? <span title="Fanvue indica que está conectado ahora." className="rounded-full bg-emerald-400/10 px-2 py-1 text-xs text-emerald-300">En línea</span> : null}{fan.isFollower ? <span title="Te sigue actualmente en Fanvue." className="rounded-full bg-sky-400/10 px-2 py-1 text-xs text-sky-300">Seguidor</span> : null}{subscriptionState ? <span title={subscriptionState.description} className="rounded-full bg-violet-400/10 px-2 py-1 text-xs text-violet-300">{subscription?.isFreeTrial ? "Prueba gratuita" : subscriptionState.label}</span> : fan.isExpiredSubscriber ? <span title="Tuvo una suscripción y todavía es contactable." className="rounded-full bg-violet-400/10 px-2 py-1 text-xs text-violet-300">Vencido</span> : null}{fan.isTopSpender ? <span title="Fanvue lo clasifica entre tus fans con mayor gasto histórico." className="rounded-full bg-amber-400/10 px-2 py-1 text-xs text-amber-300">VIP</span> : null}</div></td>
