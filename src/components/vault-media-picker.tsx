...
import { Check, Expand, Film, ImageIcon, LoaderCircle, Play, Search, X } from "lucide-react";
...
export function VaultMediaPicker({ selectedUuids, remaining, onClose, onAdd }: { selectedUuids: string[]; remaining: number; onClose: () => void; onAdd: (media: AttachedMedia[]) => void }) {
  ...
  const [previewing, setPreviewing] = useState<VaultMedia | null>(null);
  ...
  useEffect(() => { const close = (event: KeyboardEvent) => { if (event.key === "Escape" && !previewing) onClose(); }; window.addEventListener("keydown", close); return () => window.removeEventListener("keydown", close); }, [onClose, previewing]);
  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center bg-black/75 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Bóveda de Fanvue">
    <div className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-white/10 bg-[#15171d] shadow-2xl shadow-black/70">
      ...
      <div className="min-h-64 flex-1 overflow-y-auto p-4">{error ? <div className="grid min-h-64 place-items-center text-center"><div><p className="text-sm text-red-300">{error}</p><button type="button" onClick={() => void load()} className="mt-3 rounded-lg border border-white/10 px-3 py-2 text-xs text-zinc-300">Reintentar</button></div></div> : loading && !cursor ? <div className="grid min-h-64 place-items-center text-center"><div><LoaderCircle className="mx-auto size-7 animate-spin text-violet-400" /><p className="mt-3 text-xs text-zinc-500">Cargando {type === "image" ? "fotos" : type === "video" ? "videos" : "archivos"}…</p></div></div> : available.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">{available.map((item) => { const chosen = selected.includes(item.uuid); return <div key={item.uuid} className={`group relative overflow-hidden rounded-xl border text-left transition ${chosen ? "border-violet-400 ring-2 ring-violet-400/20" : "border-white/8 hover:border-white/20"}`}><button type="button" onClick={() => toggle(item.uuid)} aria-label={`${chosen ? "Deseleccionar" : "Seleccionar"} ${item.name}`} className="block w-full text-left"><div className="relative aspect-square bg-black/30"><VaultPreview item={item} />{item.mediaType === "video" ? <span className="pointer-events-none absolute inset-0 grid place-items-center"><span className="grid size-9 place-items-center rounded-full bg-black/60 text-white backdrop-blur"><Play className="ml-0.5 size-4 fill-current" /></span></span> : null}{chosen ? <span className="absolute right-2 top-2 grid size-7 place-items-center rounded-full bg-violet-500 text-white"><Check className="size-4" /></span> : null}</div><p className="truncate px-2 py-2 text-[10px] text-zinc-400" title={item.name}>{item.name}</p></button><button type="button" onClick={() => setPreviewing(item)} aria-label={`Ver ${item.name} en grande`} className="absolute left-2 top-2 z-20 grid size-8 place-items-center rounded-full border border-white/10 bg-black/70 text-white backdrop-blur transition hover:bg-violet-500 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100"><Expand className="size-4" /></button></div>; })}</div> : <div className="grid min-h-64 place-items-center text-center text-sm text-zinc-500">No encontramos fotos o videos disponibles.</div>}{loading && cursor ? <div className="mt-4 flex justify-center"><LoaderCircle className="size-5 animate-spin text-violet-400" /></div> : null}{cursor && !loading ? <button type="button" onClick={() => void load(cursor, true)} className="mx-auto mt-5 block rounded-xl border border-white/10 px-4 py-2 text-xs text-zinc-300 hover:bg-white/5">Cargar más</button> : null}</div>
    ...
    {previewing?.localUrl ? <MediaLightbox src={previewing.localUrl} type={previewing.mediaType === "video" ? "video" : "image"} name={previewing.name} poster={previewing.thumbnailUrl} onClose={() => setPreviewing(null)} /> : null}
  </div>;
}
