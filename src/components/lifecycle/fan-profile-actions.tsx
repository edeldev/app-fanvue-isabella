...
export function FanProfileActions({ fanId, stage, override, automationPaused, tags, notes }: Props) {
  ...
  return <div className="space-y-5">
    <section className="rounded-2xl border border-white/8 bg-white/[.025] p-5"><h2 className="font-semibold text-white">Control del ciclo</h2><label className="mt-4 block text-xs text-zinc-500">Etapa manual</label><select disabled={busy} value={override ?? ""} onChange={(event) => mutate({ action: "SET_STAGE", stage: event.target.value || null }, event.target.value ? "Etapa fijada manualmente" : "Cálculo automático restaurado")} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-[#171920] px-3 text-sm text-white outline-none focus:border-violet-400/50"><option value="">Automática · {lifecyclePresentation[stage].label}</option>{lifecycleStages.map((item) => <option key={item} value={item}>{lifecyclePresentation[item].label}</option>)}</select><p className="mt-2 text-[11px] leading-5 text-zinc-600">Si quitas la selección, el sistema vuelve a calcularla con datos reales.</p><button disabled={busy} onClick={() => mutate({ action: "SET_AUTOMATION_PAUSE", paused: !automationPaused }, automationPaused ? "Automatización reanudada" : "Automatización pausada")} className={`mt-4 flex w-full items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold transition ${automationPaused ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-300" : "border-amber-400/20 bg-amber-400/10 text-amber-300"}`}>{automationPaused ? <Play className="size-4"/> : <Pause className="size-4"/>}{automationPaused ? "Reanudar automatización" : "Pausar automatización"}</button></section>
