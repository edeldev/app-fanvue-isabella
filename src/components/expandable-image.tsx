...
import { Expand, ImageOff, LoaderCircle, X } from "lucide-react";
...
export function ExpandableImage({ src, alt, className = "" }: { src: string; alt: string; className?: string }) {
  ...
  return <>
    <button ref={trigger} type="button" disabled={status !== "loaded"} aria-label={`Ampliar ${alt}`} onClick={() => setOpen(true)} className="group relative block w-full overflow-hidden bg-white/[.035] text-left disabled:cursor-default">
