...
export function WorkflowEditor({ value, templates, publishedWorkflows, busy, onClose, onSave }: Props) {
  const initial: WorkflowDefinitionInput = value ? {
    name: value.name, priority: value.priority, isPrimary: value.isPrimary, steps: normalizeStepKeys(value.steps),
  } : { name: "", priority: 100, isPrimary: true, steps: normalizeStepKeys([emptyStep("END")]) };
  const form = initial;
...
function StepConfig({ step, stepIndex, steps, templates, publishedWorkflows, onChange }: { step: WorkflowDefinitionInput["steps"][number]; stepIndex: number; steps: WorkflowDefinitionInput["steps"]; templates: TemplateOption[]; publishedWorkflows: Pick<WorkflowView, "id" | "name">[]; onChange: (patch: Partial<typeof step>) => void }) {
  ...
  if (step.type === "CONDITION") { const targets = steps.slice(stepIndex + 1); return <div className="mt-3 grid gap-2"><select value={String(step.config.condition ?? "IS_FOLLOWER")} onChange={(event) => onChange({ config: { ...step.config, condition: event.target.value } })} className="w-full rounded-lg border border-white/8 bg-[#20222b] px-3 py-2 text-xs text-zinc-300"><option value="IS_FOLLOWER">Es seguidor</option><option value="IS_SUBSCRIBER">Tiene suscripción activa</option><option value="HAS_PURCHASED">Ha realizado una compra</option><option value="IS_TOP_SPENDER">Es VIP</option></select><div className="grid gap-2 sm:grid-cols-2"><BranchSelect label="Si cumple" value={String(step.config.trueTargetKey ?? "")} targets={targets} onChange={(value) => onChange({ config: { ...step.config, trueTargetKey: value } })} /><BranchSelect label="Si no cumple" value={String(step.config.falseTargetKey ?? "")} targets={targets} onChange={(value) => onChange({ config: { ...step.config, falseTargetKey: value } })} /></div></div>; }
