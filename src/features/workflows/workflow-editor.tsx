...
function StepConfig({ step, stepIndex, steps, templates, publishedWorkflows, onChange }: { step: WorkflowDefinitionInput["steps"][number]; stepIndex: number; steps: WorkflowDefinitionInput["steps"]; templates: TemplateOption[]; publishedWorkflows: Pick<WorkflowView, "id" | "name">[]; onChange: (patch: Partial<typeof step>) => void }) {
  if (step.type === "SEND_MESSAGE" || step.type === "SEND_PPV") {
    const compatible = templates.filter((template) => step.type === "SEND_PPV" ? Boolean(template.priceMinor && template.previewUuid) : !template.priceMinor);
    const replyPauseMode = ["CUSTOM", "DISABLED"].includes(String(step.config.replyPauseMode)) ? String(step.config.replyPauseMode) : "GLOBAL";
