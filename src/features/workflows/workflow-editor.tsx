...
export function WorkflowEditor({ value, templates, publishedWorkflows, busy, onClose, onSave }: Props) {
  ...
  } : { name: "", priority: 100, isPrimary: true, triggerEvent: "MANUAL", reentryPolicy: "ONCE" as const, reentryDelayDays: null, sendWindowEnabled: false, sendWindowTimezone: "America/Monterrey", sendWindowStartMinute: 540, sendWindowEndMinute: 1320, sendWindowDays: [0, 1, 2, 3, 4, 5, 6], sendLimitsEnabled: false, maxMessagesPerHour: 30, maxMessagesPerDay: 200, minMinutesBetweenFanMessages: 60, pauseOnFanReply: false, replySilenceMinutes: 60, replyAttributionHours: 24, goalType: "NONE" as const, goalAmountMinor: null, steps: normalizeStepKeys([emptyStep("END")]) };
  const form = initial;
...
function StepConfig({ step, stepIndex, steps, templates, publishedWorkflows, onChange }: { step: WorkflowDefinitionInput["steps"][number]; stepIndex: number; steps: WorkflowDefinitionInput["steps"]; templates: TemplateOption[]; publishedWorkflows: Pick<WorkflowView, "id" | "name">[]; onChange: (patch: Partial<typeof step>) => void }) {
  ...
  if (step.type === "CONDITION") return <ConditionConfig step={step} targets={steps.slice(stepIndex + 1)} onChange={onChange} />;
...
function ConditionConfig({ step, targets, onChange }: { step: WorkflowDefinitionInput["steps"][number]; targets: WorkflowDefinitionInput["steps"]; onChange: (patch: Partial<typeof step>) => void }) {
...
function NextStepSelect({ step, targets, onChange }: { step: WorkflowDefinitionInput["steps"][number]; targets: WorkflowDefinitionInput["steps"]; onChange: (value: string) => void }) {
  return <label className="mt-2 block text-[11px] text-zinc-600">Después continuar en <select value={String(step.config.nextTargetKey ?? "")} onChange={(event) => onChange(event.target.value)} className="ml-2 rounded-lg border border-white/8 bg-[#20222b] px-2 py-1.5 text-xs text-zinc-400"><option value="">Siguiente paso en orden</option>{targets.map((target) => <option key={String(target.config.stepKey)} value={String(target.config.stepKey)}>{target.name}</option>)}</select></label>;
}
