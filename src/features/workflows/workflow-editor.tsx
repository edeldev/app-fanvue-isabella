...
function StepConfig({ step, stepIndex, steps, templates, publishedWorkflows, onChange }: { step: WorkflowDefinitionInput["steps"][number]; stepIndex: number; steps: WorkflowDefinitionInput["steps"]; templates: TemplateOption[]; publishedWorkflows: Pick<WorkflowView, "id" | "name">[]; onChange: (patch: Partial<typeof step>) => void }) {
  ...
  if (step.type === "CONDITION") return <ConditionConfig step={step} targets={steps.slice(stepIndex + 1)} firstTargetNumber={stepIndex + 2} templates={templates} onChange={onChange} />;
...
function ConditionConfig({ step, targets, firstTargetNumber, templates, onChange }: { step: WorkflowDefinitionInput["steps"][number]; targets: WorkflowDefinitionInput["steps"]; firstTargetNumber: number; templates: TemplateOption[]; onChange: (patch: Partial<typeof step>) => void }) {
