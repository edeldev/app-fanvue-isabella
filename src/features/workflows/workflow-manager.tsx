...
export function WorkflowManager({ workflows, templates }: { workflows: WorkflowView[]; templates: TemplateOption[] }) {
  ...
  function remove(workflow: WorkflowView) {
    const warning = workflow.status === "PUBLISHED"
      ? `¿Eliminar definitivamente “${workflow.name}”? Está publicado, pero nunca se ha utilizado.`
      : `¿Eliminar el borrador “${workflow.name}”?`;
    if (!window.confirm(warning)) return;
    void request(`/api/workflows/${workflow.id}`, { method: "DELETE" }, "Workflow eliminado correctamente.");
  }
...
function translateError(message: string) {
  const errors: Record<string, string> = { WORKFLOW_IMMUTABLE: "Los flujos publicados no se pueden editar.", WORKFLOW_NOT_FOUND: "El flujo no existe.", WORKFLOW_TEMPLATE_NOT_FOUND: "Una plantilla seleccionada no existe.", WORKFLOW_TARGET_NOT_FOUND: "El flujo de destino debe estar publicado.", WORKFLOW_HAS_HISTORY: "Este workflow ya tiene historial y no puede eliminarse; debe archivarse." };
