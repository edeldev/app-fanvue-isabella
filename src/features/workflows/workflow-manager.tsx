...
export function WorkflowManager({ workflows, templates }: { workflows: WorkflowView[]; templates: TemplateOption[] }) {
  ...
  async function request(url: string, options: RequestInit) {
    ...
    try {
      ...
      setMessage({ text: "Cambios guardados correctamente." });
      setEditor(undefined);
      router.refresh();
    } catch (error) {
...
function translateError(message: string) {
  const errors: Record<string, string> = { WORKFLOW_IMMUTABLE: "Los flujos publicados no se pueden editar.", WORKFLOW_NOT_FOUND: "El flujo no existe.", WORKFLOW_TEMPLATE_NOT_FOUND: "Una plantilla seleccionada no existe.", WORKFLOW_TARGET_NOT_FOUND: "El flujo de destino debe estar publicado." };
