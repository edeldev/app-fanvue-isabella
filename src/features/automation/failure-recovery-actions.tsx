...
export function FailureRecoveryActions({ enrollmentId, workflowName, stepName }: { enrollmentId: string; workflowName: string; stepName: string }) {
  ...
  async function confirm() {
    ...
    try {
      ...
      setAction(null);
      enqueueSnackbar(action === "cancel" ? "Workflow cancelado definitivamente." : "Recuperación del workflow programada correctamente.", { variant: "success" });
      router.refresh();
    } catch (caught) {
      enqueueSnackbar(caught instanceof Error ? caught.message : "No se pudo recuperar el workflow.", { variant: "error" });
    } finally {
      setBusy(false);
    }
  }
  ...
  return <>
    ...
    {selected && SelectedIcon ? <div role="dialog" aria-modal="true" aria-labelledby="recovery-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setAction(null); }} className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-white/10 bg-[#181a21] shadow-2xl shadow-black/60">
        ...
        <div className="p-5"><h2 id="recovery-title" className="text-lg font-semibold text-white">{selected.title}</h2><p className="mt-2 text-sm leading-6 text-zinc-400">{selected.description}</p><div className="mt-4 rounded-xl bg-black/20 p-3 text-xs text-zinc-500"><p className="font-medium text-zinc-300">{workflowName}</p><p className="mt-1">Paso fallido: {stepName}</p></div></div>
